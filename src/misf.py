...
class MISF():
    def __init__(self, config):
        ...
        # 根据模型模式（训练或测试）选择日志文件路径
        if config.MODE == 2:  # 测试模式
            log_file = os.path.join(config.PATH, 'log/test_60.log')
        else:  # 训练模式
            log_file = os.path.join(config.PATH, 'log/train.log')

        # 如果日志文件已存在，覆盖该文件
        file_handler = logging.FileHandler(log_file, mode='w')  # mode='w' 表示覆盖文件
        ...
        if self.config.MODE == 2:
            ...
            self.logger.info('test dataset:'.format(len(self.test_dataset)))
        else:
            ...
            self.logger.info('train dataset:{}'.format(len(self.train_dataset)))
            self.logger.info('eval dataset:{}'.format(len(self.val_dataset)))
        ...
        while(keep_training):
            ...
            for items in train_loader:
                ...
                if iteration % self.config.TRAIN_SAMPLE_INTERVAL == 0:
                    #######################################
                    self.logger.info(f"Saving samples at iteration {iteration}...")
                ...
                if iteration % self.config.SAVE_INTERVAL == 0:
                    #######################################
                    self.logger.info(f"Saving model at iteration {iteration}...")
                ...
                if iteration % self.config.EVAL_INTERVAL == 0:
                    #######################################
                    self.logger.info(f"Evaluating model at iteration {iteration}...")

                    print('\nstart eval...\n')
                    cur_psnr = self.eval()
                    #######################################
                    self.logger.info(f"Current PSNR: {cur_psnr}")
                    ...
                    if cur_psnr > max_psnr:
                        ...
                        self.logger.info('---increase-iteration:{}'.format(iteration))
    ...
    def eval(self):
        ...
        lpips_list = []
        #######################################
        self.logger.info(f"Average PSNR: {np.average(psnr_all)}")
        ...
        with torch.no_grad():
            for items in val_loader:
                ...
                self.logger.info('psnr:{}/{}  ssim:{}/{} l1:{}/{}  lpips{}/{}  {}/{}'.format(psnr, np.average(psnr_all),
                                                                                   ssim, np.average(ssim_all),
                                                                                   l1_loss, np.average(l1_list),
                                                                                   pl, np.average(lpips_list),
                                                                                   len(psnr_all), len(self.val_dataset)))
            ...
            #######################################
            self.logger.info('iteration:{} ave_psnr:{}  ave_ssim:{} ave_l1:{}  ave_lpips:{}'.format(
                iteration,
                np.average(psnr_all),
                np.average(ssim_all),
                np.average(l1_list),
        ...
        with torch.no_grad():
            for items in test_loader:
                ...
                print("psnr:{}/{}  ssim:{}/{} l1:{}/{}  lpips:{}/{}  {}".format(psnr, np.average(psnr_list),
                                                                                ...
                                                                                len(ssim_list)))
                ###########################################################################################
                self.logger.info("psnr:{}/{}  ssim:{}/{} l1:{}/{}  lpips:{}/{}  {}".format(psnr, np.average(psnr_list),
                                                                                ssim, np.average(ssim_list),
                                                                                l1_loss, np.average(l1_list),
                                                                                pl, np.average(lpips_list),
                                                                                len(ssim_list)))
            ...
            print('psnr_ave:{} ssim_ave:{} l1_ave:{} lpips:{}'.format(np.average(psnr_list),
                                                                                 ...
                                                                                 np.average(lpips_list)))
            ###########################################################################################
            self.logger.info('psnr_ave:{} ssim_ave:{} l1_ave:{} lpips:{}'.format(np.average(psnr_list),
                                                                                 np.average(ssim_list),
                                                                                 np.average(l1_list),
                                                                                 np.average(lpips_list)))
